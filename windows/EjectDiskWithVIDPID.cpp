//根据设备的VID和PID自动弹出USB设备
//根据设备的VID和PID查找设备的盘符
DEVINST GetDrivesDevInstByDeviceNumber(long DeviceNumber)
{
	//const GUID* guid = &GUID_DEVINTERFACE_CDROM; //这里可以设置自动弹出CDROM
	const GUID* guid = &GUID_DEVINTERFACE_DISK;    //这里可以设置自动弹出DISK包括U盘
...
void usbEjectDevice(unsigned vid, unsigned pid)
{
	...
	for (TCHAR driveletter = _T('A'); driveletter <= _T('Z'); driveletter++)
	{
		...
		//_tprintf(driveletter);
		// We are only interested in CDROM drives.
		drivepath[0] = driveletter;
		if (DRIVE_REMOVABLE != GetDriveType(drivepath))  //可移动设备 CDROM和USB设备U盘
			continue;
		...
		if (DevInst)
		{
			if (matchDevInstToUsbDevice(DevInst, vid, pid)) {
				...
				std::cout << "not found it " << endl;
			}
		}
		else {
			std::cout << "DevInst null " << endl;
		}
	}
}